...
-- Source JSON: C:/Projects/superastro/debug_purpose_path_ru_160_20260223_143219_mini_topiccore.json
...
UPDATE knowledge_items
SET text = 'Натальный маркер (natal.house.9.ruler.any): это подсказка о стиле реакции и типичных сценариях. Практика: отследи, как это проявляется 7 дней (триггеры → реакция → результат) и выбери одну настройку поведения (границы, темп, коммуникация, режим восстановления).', is_active = 1, priority = 140, meta_json = '{}'
WHERE key = 'natal.house.9.ruler.any' AND locale = 'ru-RU' AND topic_category = 'purpose_path';

INSERT INTO knowledge_items (key, topic_category, locale, text, priority, created_at, is_active, meta_json)
SELECT 'natal.house.9.ruler.any', 'purpose_path', 'ru-RU', 'Натальный маркер (natal.house.9.ruler.any): это подсказка о стиле реакции и типичных сценариях. Практика: отследи, как это проявляется 7 дней (триггеры → реакция → результат) и выбери одну настройку поведения (границы, темп, коммуникация, режим восстановления).', 140, CURRENT_TIMESTAMP, 1, '{}'
WHERE NOT EXISTS (
  SELECT 1 FROM knowledge_items
  WHERE key = 'natal.house.9.ruler.any' AND locale = 'ru-RU' AND topic_category = 'purpose_path'
);
