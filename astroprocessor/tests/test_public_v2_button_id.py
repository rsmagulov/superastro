# ============================================================
# File: astroprocessor/tests/test_public_v2_button_id.py  (NEW)
# ============================================================
...
def test_v2_button_id_maps_to_topics_when_topic_categories_empty(monkeypatch):
    ...
    def fake_build_knowledge_blocks(*, natal_data: dict, tone_namespace: str = "natal"):
        return []  # не важно, т.к. ниже мы замокаем batch
    ...
    async def fake_interpret_topics_with_blocks_batch(
        *,
        ...
        return {
            t: {"final_text": f"Topic: {t}", "raw_blocks": [{"block_id": "x", "knowledge_item_id": 1, "key": "k", "priority": 1, "created_at": None, "text": "t"}]}
    ...
    assert [t["topic_category"] for t in body["topics"]] == ["personality_core", "career"]
    assert body["meta"]["button_id"] == "btn_core_pack"
    assert body["meta"]["topics"] == ["personality_core", "career"]


def test_v2_explicit_topic_categories_override_button_id(monkeypatch):
    import app.routers.public_v2 as public_v2
    ...
    payload = {
        "button_id": "btn_core_pack",  # маппится в 2 темы, но мы задаём явно одну
    ...
    assert [t["topic_category"] for t in body["topics"]] == ["career"]
    assert body["meta"]["topics"] == ["career"]
