# astroprocessor/tests/test_e2e_v2_debug2_hits_include_sign_present.py
...
def _seed_kb_min_for_signs(path: Path) -> None:
    conn = sqlite3.connect(str(path))
    try:
        rows = [
            ...
            # sign.present (GLOBAL)
            ("natal.sign.capricorn.present", "", "ru-RU", "capricorn present", 120, 1),
            ("natal.sign.pisces.present", "", "ru-RU", "pisces present", 120, 1),
        ]
        conn.executemany(
            """
...
def _stub_natal_data() -> dict:
    return {
        ...
        "planets": {
            "sun": {"sign": "capricorn", "house": 1},  # ensures capricorn present (personal planet)
            "moon": {"sign": "pisces", "house": 3},    # ensures pisces present (personal planet)
            "mercury": {"sign": "aries", "house": 2},
            "venus": {"sign": "taurus", "house": 4},
            "mars": {"sign": "gemini", "house": 5},
        },
        "angles": {"asc": {"sign": "capricorn"}, "mc": {"sign": "aries"}},
...
def test_e2e_debug2_hits_include_sign_present(tmp_path: Path, monkeypatch, topic: str) -> None:
    import app.routers.public_v2 as public_v2

    kb_path = tmp_path / "kb_signs.db"
    _create_kb_db(kb_path)
    _seed_kb_min_for_signs(kb_path)
    ...
    try:
        ...
        assert any(k == "natal.sign.capricorn.present" for k in keys) or any(k == "natal.sign.pisces.present" for k in keys), (
            f"Expected natal.sign.<s>.present in hits_sample, got keys={keys}"
        )
    finally:
