...
# ============================================================
# Text decoding helpers (best-effort)
# ============================================================

_ZERO_WIDTH_RE = re.compile(r"[\u200B-\u200F\uFEFF]")
_CTRL_RE = re.compile(r"[\x00-\x08\x0b\x0c\x0e-\x1f]")

# common mojibake markers (UTF-8 bytes decoded as cp1251/latin1 etc.)
_MOJIBAKE_RE = re.compile(r"(?:Р[А-Яа-я]|С[А-Яа-я]|вЂ[“”«»—–…]|Ð|Ñ)")

def _looks_like_mojibake(s: str) -> bool:
    if not s:
        return False
    return len(_MOJIBAKE_RE.findall(s)) >= 3

def _try_unmojibake_cp1251(s: str) -> str:
    """
    Reverse common case: UTF-8 bytes were decoded as CP1251 earlier,
    producing 'Р…/С…/вЂ…' sequences. Try to restore.
    """
    try:
    ...
    # accept only if markers reduced
...
def _decode_bytes_best_effort(data: bytes) -> str:
    """
    Decode unknown text bytes with a deterministic heuristic.
    Tries UTF-8(+BOM), UTF-16 (LE/BE), and common Russian encodings.
    """
    if not data:
        return ""
    ...
    for enc in candidates:
        ...
        score = 0.0
        # NULs in decoded string usually mean wrong codec
        score += 50.0 * s.count("\x00")
        # mojibake markers are a strong signal
        score += 10.0 * len(_MOJIBAKE_RE.findall(s))
        # if bytes had many NULs, prefer utf-16*
    ...
    if best_s is None:
        best_s = data.decode("utf-8", errors="ignore")
...
def _read_fb2_to_text(path: Path) -> str:
    """
    Robust FB2 -> plain text.

    Reads bytes, lets ElementTree handle XML prolog encoding,
    extracts only <p> paragraphs from <body>, ignores <binary>,
    and avoids falling back to raw XML.
    """
    data = path.read_bytes()

    # Strip UTF-8 BOM if present (so ET doesn't get confused in edge cases)
    if data.startswith(b"\xef\xbb\xbf"):
        data = data[3:]

    root = ET.fromstring(data)

    # FB2 namespace handling
    ns = ""
    if root.tag.startswith("{"):
        ns = root.tag.split("}")[0] + "}"

    def _t(el: Any) -> str:
        # itertext() pulls text inside nested tags, not the tags themselves
        return ("".join(el.itertext()) if el is not None else "").strip()

    paragraphs: list[str] = []

    # Preferred FB2 structure: <body> ... <p> ... </p>
    for p in root.findall(f".//{ns}body//{ns}p"):
        s = _t(p)
        if s:
            paragraphs.append(s)

    # If no paragraphs found, fall back to all visible text except <binary>
    if not paragraphs:
        for el in root.iter():
    ...
    # join as book-like text
    text = "\n\n".join(paragraphs)

    # light cleanup (avoid huge whitespace blobs)
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    text = re.sub(r"[ \t]+", " ", text)
    text = re.sub(r"\n{3,}", "\n\n", text).strip()
    text = _normalize_text_ingest(text)
    return text
...
def _read_epub_to_text(path: Path) -> str:
    ...
    with zipfile.ZipFile(path, "r") as z:
        html_names = sorted([n for n in z.namelist() if n.lower().endswith((".xhtml", ".html", ".htm"))])
        for name in html_names:
            try:
                b = z.read(name)
            except Exception:
                continue
            s = _decode_bytes_best_effort(b)
            t = _html_to_text(s)
            t = _normalize_text_ingest(t)
            if t.strip():
...
def _read_rtf_to_text(path: Path) -> str:
    ...
    s = re.sub(r"[ \t]+", " ", s)
    s = re.sub(r"\n\s+\n", "\n\n", s)
    return _normalize_text_ingest(s)
...
def _convert_to_markdown(src: Path) -> tuple[str, str]:
    ...
    if ext == ".md":
        return title, _normalize_text_ingest(_decode_bytes_best_effort(src.read_bytes()))
    if ext == ".txt":
        return title, _normalize_text_ingest(_decode_bytes_best_effort(src.read_bytes()))
